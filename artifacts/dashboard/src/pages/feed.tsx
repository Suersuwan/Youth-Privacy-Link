      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-4 border-b border-border">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-4 mr-4 font-mono text-xs text-muted-foreground">
            ...
            <div className="flex flex-col">
              <span className="uppercase tracking-widest text-[10px]">Active SUBS</span>
      ...
      <div className="flex-1 overflow-auto border border-border rounded bg-card p-4 relative font-mono shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]" ref={containerRef}>
        ...
        <div className="space-y-1 relative z-10 text-sm">
          {liveEvents.length === 0 ? (
            ...
            liveEvents.map((evt) => (
              <div key={evt.id} className="flex items-center gap-4 py-2.5 px-3 border-b border-border/40 hover:bg-secondary/40 transition-colors group">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <div className="text-muted-foreground w-20 shrink-0 text-[10px] uppercase tracking-wider cursor-help opacity-70 group-hover:opacity-100 transition-opacity">
                ...
                <div className="flex items-center gap-2 w-32 shrink-0">
                  ...
                  <span className="text-primary/90 font-medium group-hover:text-primary transition-colors">{truncateId(evt.id)}</span>
                </div>
                ...
                <div className="flex-1 flex gap-3 items-center">
                  <span className="bg-primary/10 text-primary px-2 py-0.5 rounded text-[11px] font-bold border border-primary/20 uppercase tracking-wider shadow-[0_0_8px_rgba(0,255,209,0.1)]">
                    TYPE_{evt.eventType}
                  </span>
                  {evt.guildId && (

...
import { Activity, BarChart3, ShieldCheck, ShieldAlert } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-background text-foreground font-mono dark">
      ...
      <aside className="w-64 border-r border-border bg-card flex flex-col z-10 relative shadow-[4px_0_24px_rgba(0,0,0,0.5)]">
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
          <Link href="/" className={`flex items-center gap-3 px-4 py-3 rounded transition-all duration-200 ${location === "/" ? "bg-primary/10 text-primary border border-primary/30 shadow-[inset_0_0_12px_rgba(0,255,209,0.1)]" : "text-muted-foreground hover:bg-secondary hover:text-foreground border border-transparent"}`}>
            <Activity className="w-4 h-4" />
            <span className="text-sm font-semibold tracking-wide uppercase">Live Feed</span>
          </Link>
          
          <Link href="/stats" className={`flex items-center gap-3 px-4 py-3 rounded transition-all duration-200 ${location === "/stats" ? "bg-primary/10 text-primary border border-primary/30 shadow-[inset_0_0_12px_rgba(0,255,209,0.1)]" : "text-muted-foreground hover:bg-secondary hover:text-foreground border border-transparent"}`}>
            <BarChart3 className="w-4 h-4" />
            <span className="text-sm font-semibold tracking-wide uppercase">Telemetry</span>
          </Link>
        </nav>
